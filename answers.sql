...
SELECT m.name, b.name, headquarters FROM brands AS b JOIN models AS m USING (brand_id) WHERE m.name = 'Mustang' AND b.name = 'Ford';
...
SELECT COUNT(*) FROM brands AS b JOIN models AS m USING (brand_id) WHERE b.name = 'Ford';
...
SELECT * FROM models WHERE model_id BETWEEN 15 and 24 ORDER BY year ASC LIMIT 10;
...
SELECT b.name, m.year, m.name FROM brands AS b LEFT JOIN models AS m ON b.brand_id = m.brand_id WHERE m.name  IS NOT NULL ORDER BY b.name;